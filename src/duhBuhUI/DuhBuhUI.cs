...
public sealed class DuhBuhUI
{
    ...
    {
        RegisterCategory(category);
        _defaults[variableName] = NormalizeColor(defaultValue) ?? "#FFFFFFFF";
    ...
    {
        RegisterCategory(category);
        _buttons.Add(new ButtonDefinition { Title = title, Description = description, ButtonText = buttonText, Color = color, Category = category, Callback = callback });
    }
    ...
    private void BuildCategory(StackPanel panel, string category, string theme)
    {
        for (int i = 0; i < _titles.Count; i++)
        {
            if (_titles[i].Category == category)
                panel.Children.Add(new TextBlock { Text = _titles[i].Title, FontSize = 18, FontWeight = FontWeights.SemiBold, Foreground = ThemeBrush(theme, "PrimaryText"), Margin = new Thickness(0, 10, 0, 12) });
        }
        ...
        for (int i = 0; i < _controls.Count; i++)
        {
            ...
            if (d.Type == "toggle")
            {
                ...
                box.Children.Insert(0, new CheckBox { Content = d.Title, IsChecked = Read(d.Key, (bool)d.DefaultValue), FontSize = 14, Foreground = ThemeBrush(theme, "PrimaryText"), Margin = new Thickness(0, 5, 0, 2), Tag = d.Key });
            ...
            else if (d.Type == "textbox")
            {
                ...
                box.Children.Insert(1, new TextBox { Text = Read(d.Key, (string)d.DefaultValue), AcceptsReturn = d.Multiline, TextWrapping = TextWrapping.Wrap, MinHeight = d.Multiline ? 65 : 30, Tag = d.Key });
                panel.Children.Add(box);
            }
            ...
            {
                BuildColorControl(panel, d, theme);
            }
            else if (d.Type == "date" || d.Type == "time" || d.Type == "datetime")
            {
                ...
                box.Children.Insert(0, new TextBlock { Text = d.Title, FontSize = 14, Foreground = ThemeBrush(theme, "PrimaryText") });
                string current = Read(d.Key, (string)d.DefaultValue);
                if (d.Type == "date" || d.Type == "datetime") box.Children.Insert(1, new DatePicker { Tag = d.Key, SelectedDate = ParseDate(current) });
                if (d.Type == "time" || d.Type == "datetime") box.Children.Insert(2, new TextBox { Tag = d.Type == "datetime" ? d.Key + "::time" : d.Key, Text = ExtractTime(current), MinWidth = 100, Margin = new Thickness(0, 4, 0, 0) });
                panel.Children.Add(box);
            }
        }
    ...
    private void BuildColorControl(StackPanel panel, ControlDefinition d, string theme)
    {
        ...
        {
            string c = NormalizeColor(hex.Text);
            if (c != "") { hex.Text = c; SetSwatch(swatch, c); }
        };

        row.Children.Add(swatch);
        row.Children.Add(hex);
        row.Children.Add(choose);
        box.Children.Insert(1, row);
        box.Children.Insert(2, new TextBlock { Text = "Use the swatch or enter #RRGGBB / #AARRGGBB", Foreground = ThemeBrush(theme, "SecondaryText"), Margin = new Thickness(0, 2, 0, 0) });
        panel.Children.Add(box);
    }
    ...
    {
        string initial = NormalizeColor(target.Text);
        if (initial == "") initial = "#FFFFFFFF";
        ...
        TextBlock heading = new TextBlock { Text = "Choose a notification colour", FontSize = 16, FontWeight = FontWeights.SemiBold, Foreground = ThemeBrush(theme, "PrimaryText"), Margin = new Thickness(0, 0, 0, 10) };
        ...
        TextBlock previewText = new TextBlock { Text = initial, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center, Foreground = Brushes.White, FontWeight = FontWeights.Bold };
        preview.Child = previewText;
        SetPreview(preview, previewText, initial);
        ...
        string[] colours = new[]
        {
            ...
            "#FFFFBE0B", "#FF2A9D8F", "#FF06D6A0", "#FF118AB2", "#FFEF476F", "#FF6C757D"
        };

        for (int i = 0; i < colours.Length; i++)
        {
            string colour = colours[i];
            Button swatchButton = new Button { Width = 38, Height = 32, Margin = new Thickness(3), Tag = colour, ToolTip = colour, Padding = new Thickness(0) };
            swatchButton.Background = BrushFromHex(colour);
            swatchButton.BorderBrush = ThemeBrush(theme, "SecondaryText");
            swatchButton.Click += delegate(object sender, RoutedEventArgs e)
            {
        ...
        TextBox custom = new TextBox { Text = initial, Width = 150, VerticalContentAlignment = VerticalAlignment.Center };
        ...
        customRow.Children.Add(applyCustom);
        root.Children.Add(customRow);

        custom.TextChanged += delegate
        {
    ...
    {
        preview.Background = BrushFromHex(colour);
        text.Text = colour;
        Color c;
        try { c = (Color)ColorConverter.ConvertFromString(colour); }
        catch { c = Colors.White; }
        double luminance = (0.299 * c.R + 0.587 * c.G + 0.114 * c.B) / 255.0;
        text.Foreground = luminance > 0.6 ? Brushes.Black : Brushes.White;
    }

    private void SetSwatch(Button button, string value)
    {
        button.Background = BrushFromHex(value);
        button.BorderBrush = new SolidColorBrush(Color.FromArgb(180, 120, 120, 120));
    }
    ...
    {
        DateTime parsed;
        if (DateTime.TryParse(value, CultureInfo.InvariantCulture, DateTimeStyles.None, out parsed)) return parsed.ToString("HH:mm", CultureInfo.InvariantCulture);
        return value;
    }

    private object ReadObject(string key, object fallback)
    {
        try { object value = _getObject(key, true); return value ?? fallback; } catch { return fallback; }
    }

    private bool Read(string key, bool fallback)
    {
        try { bool? value = _getBool(key, true); return value.HasValue ? value.Value : fallback; } catch { return fallback; }
    }

    private int Read(string key, int fallback)
    {
        try { int? value = _getInt(key, true); return value.HasValue ? value.Value : fallback; } catch { return fallback; }
    }

    private string Read(string key, string fallback)
    {
        try { string value = _getString(key, true); return value ?? fallback; } catch { return fallback; }
    }
        ...
        foreach (object childObject in LogicalTreeHelper.GetChildren(parent))
        {
            ...
            if (fe != null && Equals(fe.Tag, key))
            {
                ...
                if (date != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(time) && time != current && current.IndexOf(':') >= 0) _setGlobal(key, dateText + " " + time, true);
                    else _setGlobal(key, dateText, true);
                }
